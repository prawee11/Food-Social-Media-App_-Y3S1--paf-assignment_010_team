...
const Post = props => {
  ...
  return (
    <div className="editor mx-auto my-6 w-10/12 flex flex-col border border-gray-100 max-w-2xl postbg">
      <article className="overflow-hidden rounded-lg shadow transition hover:shadow-lg">
        ...
        <div className="bg-white p-4 sm:p-6">
          <time
            dateTime="2022-10-10"
            className=" text-start block text-xs text-gray-500"
          >
            {new Date(props.postedTime).toISOString().split("T")[0]}
          </time>

          <p className="mt-2 text-start text-sm/relaxed text-gray-500">
            {props.description}
          </p>
        </div>

        <div className="bg-white border-t">
          <CommentBox {...props} shared={shared} updateComment={updateComment} />
        </div>
        <div className="bg-white border-t">
